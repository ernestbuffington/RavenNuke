...
if ( $mode == 'newpm' ) {
	...
	include_once($phpbb_root_path . 'includes/page_header_review.'.$phpEx);
	...
	include_once($phpbb_root_path . 'includes/page_tail_review.'.$phpEx);
	die(); // Per Loki - 0001207: PM pop-up window is not properly styled and is not compliant in RavenIce
} else if ( $mode == 'read' ) {
	...
	if ( !empty($privmsg['user_icq']) ) {
		...
		$icq_img = '<a href="http://people.icq.com/people/' . $privmsg['user_icq'] . '"><img src="' . $images['icon_icq'] . '" alt="' . $lang['ICQ'] . '" title="' . $lang['ICQ'] . '" border="0" /></a>';
		$icq =  '<a href="http://people.icq.com/people/' . $privmsg['user_icq'] . '">' . $lang['ICQ'] . '</a>';
	} else {
		$icq_status_img = '';
		$icq_img = '';
		$icq = '';
	...
	/** Removed in v2.30.01 - Mantis 1319.  The setting $forum_on cannot be found in any scripts nor tables since phpnuke v6.5
	if ($forum_on) {
		make_jumpbox('viewforum.'.$phpEx);
	}
	Removed in v2.30.01.  The setting $forum_on cannot be found in any scripts nor tables since phpnuke v6.5 **/
...
if ( $row = $db->sql_fetchrow($result) ) {
	...
	do {
		...
		if ( $flag == PRIVMSGS_NEW_MAIL && $folder == 'inbox' ) {
			$msg_subject = '<span class="thick">' . $msg_subject . '</span>';
			$msg_date = '<span class="thick">' . $msg_date . '</span>';
			$msg_username = '<span class="thick">' . $msg_username . '</span>';
		}
