	...
	*/
	function display_attachment_bodies()
	{
		...
		$template->assign_vars(array(
			...
			'RULES'							=> '<a target="phpbbattachrules" onclick="window.open(\'modules.php?name=Forums&amp;file=attach_rules&amp;f=' . $u_rules_id . '\', \'phpbbattachrules\', \'height=300,resizable=yes,scrollbars=yes,width=500\');return false;" href="' . append_sid($phpbb_root_path . 'attach_rules.' . $phpEx . '?f=' . $u_rules_id) . '">' . $lang['Allowed_extensions_and_sizes'] . '</a>',
	...
	function upload_attachment()
	{
		...
		if ($this->post_attach) 
		{
			...
			{ 
				...
				$error_msg .= sprintf($lang['Invalid_filename'], htmlspecialchars($this->filename, ENT_COMPAT));
			}
			...
			if (!$error && intval($row['allow_group']) == 0)
			{
				...
				$error_msg .= sprintf($lang['Disallowed_extension'], htmlspecialchars($this->extension, ENT_COMPAT));
			} 
			...
			if (!$error && $this->page != PAGE_PRIVMSGS && $userdata['user_level'] != ADMIN && !is_forum_authed($auth_cache, $forum_id) && trim($auth_cache) != '')
			{
				...
				$error_msg .= sprintf($lang['Disallowed_extension_within_forum'], htmlspecialchars($this->extension, ENT_COMPAT));
			} 
			...
			if (!$error) 
			{
				...
				if (!$cryptic)
				{
					$this->attach_filename = html_entity_decode(trim(stripslashes($this->attach_filename)), ENT_QUOTES);
