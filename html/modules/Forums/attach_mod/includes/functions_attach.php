...
function physical_filename_already_stored($filename)
{
	...
	{
		message_die(GENERAL_ERROR, 'Could not get attachment information for filename: ' . htmlspecialchars($filename, ENT_COMPAT), '', __LINE__, __FILE__, $sql);
	}
...
function attach_mod_sql_escape($text) {
	global $db;
	return $db->sql_escape_string($text);
}
