...
if( isset($HTTP_POST_VARS['login']) || isset($HTTP_GET_VARS['login']) || isset($HTTP_POST_VARS['logout']) || isset($HTTP_GET_VARS['logout']) )
{
		...
		if( $row = $db->sql_fetchrow($result) )
		{
			...
			else
			{
				...
				if( md5($password) == $row['user_password'] && $row['user_active'] )
				{
					...
					if( $session_id )
					{
						$url = ( !empty($HTTP_POST_VARS['redirect']) ) ? str_replace('&amp;', '&', htmlspecialchars($HTTP_POST_VARS['redirect'], ENT_COMPAT)) : 'index.'.$phpEx;
				...
				elseif( $row['user_active'] )
				{
					...
					$redirect = ( !empty($HTTP_POST_VARS['redirect']) ) ? str_replace('&amp;', '&', htmlspecialchars($HTTP_POST_VARS['redirect'], ENT_COMPAT)) : '';
	...
	else if( ( isset($HTTP_GET_VARS['logout']) || isset($HTTP_POST_VARS['logout']) ) && $userdata['session_logged_in'] )
	{
		...
		{
			$url = (!empty($HTTP_POST_VARS['redirect'])) ? htmlspecialchars($HTTP_POST_VARS['redirect'], ENT_COMPAT) : htmlspecialchars($HTTP_GET_VARS['redirect'], ENT_COMPAT);
			$url = str_replace('&amp;', '&', $url);
			redirect(append_sid($url, true));
		}
		else
