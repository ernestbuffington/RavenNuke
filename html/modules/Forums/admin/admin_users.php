 ...
 *
 * Applied rules: Ernest Allen Buffington (TheGhost) 04/21/2023 5:03 PM
...
require_once("../includes/functions_selects.php");
require_once("../includes/functions_validate.php");
...
if ( $mode == 'edit' || $mode == 'save' && ( isset($HTTP_POST_VARS['username']) || isset($HTTP_GET_VARS[POST_USERS_URL]) || isset( $HTTP_POST_VARS[POST_USERS_URL]) ) )
{
		...
		$username = ( !empty($HTTP_POST_VARS['username']) ) ? phpbb_clean_username($HTTP_POST_VARS['username']) : '';
                $email = ( !empty($HTTP_POST_VARS['email']) ) ? trim(strip_tags(htmlspecialchars( $HTTP_POST_VARS['email'] , ENT_COMPAT ) )) : '';

                $password = ( !empty($HTTP_POST_VARS['password']) ) ? trim(strip_tags(htmlspecialchars( $HTTP_POST_VARS['password'] , ENT_COMPAT) )) : '';
                $password_confirm = ( !empty($HTTP_POST_VARS['password_confirm']) ) ? trim(strip_tags(htmlspecialchars( $HTTP_POST_VARS['password_confirm'] , ENT_COMPAT) )) : '';
		...
		$user_style = ( isset( $HTTP_POST_VARS['style'] ) ) ? intval( $HTTP_POST_VARS['style'] ) : $board_config['default_style'];
                $user_lang = $HTTP_POST_VARS['language'] ?: $board_config['default_lang'];
                $user_timezone = ( isset( $HTTP_POST_VARS['timezone']) ) ? doubleval( $HTTP_POST_VARS['timezone'] ) : $board_config['board_timezone'];
                $user_dateformat = ( $HTTP_POST_VARS['dateformat'] ) ? trim( $HTTP_POST_VARS['dateformat'] ) : $board_config['default_dateformat'];

                $user_avatar_local = ( isset( $HTTP_POST_VARS['avatarselect'] ) && !empty($HTTP_POST_VARS['submitavatar'] ) && $board_config['allow_avatar_local'] ) ? $HTTP_POST_VARS['avatarselect'] : ( $HTTP_POST_VARS['avatarlocal'] ?? '' );
                ...
                if( isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
                {
                        ...
                        $aim = htmlspecialchars(stripslashes($aim), ENT_COMPAT);
                        $msn = htmlspecialchars(stripslashes($msn), ENT_COMPAT);
                        $yim = htmlspecialchars(stripslashes($yim), ENT_COMPAT);

                        $website = htmlspecialchars(stripslashes($website), ENT_COMPAT);
                        $location = htmlspecialchars(stripslashes($location), ENT_COMPAT);
                        $occupation = htmlspecialchars(stripslashes($occupation), ENT_COMPAT);
                        $interests = htmlspecialchars(stripslashes($interests), ENT_COMPAT);
                        $signature = htmlspecialchars(stripslashes($signature), ENT_COMPAT);

                        $user_lang = stripslashes($user_lang);
                        $user_dateformat = htmlspecialchars(stripslashes($user_dateformat), ENT_COMPAT);
        ...
        if( isset( $HTTP_POST_VARS['submit'] ) )
        {
                ...
                else
                {
                        ...
                        $username = htmlspecialchars(stripslashes($username), ENT_COMPAT);
                        $email = stripslashes($email);
                        $password = '';
                        $password_confirm = '';

                        $icq = stripslashes($icq);
                        $aim = htmlspecialchars(str_replace('+', ' ', stripslashes($aim)), ENT_COMPAT);
                        $msn = htmlspecialchars(stripslashes($msn), ENT_COMPAT);
                        $yim = htmlspecialchars(stripslashes($yim), ENT_COMPAT);
        ...
        else if( !isset( $HTTP_POST_VARS['submit'] ) && $mode != 'save' && !isset( $HTTP_POST_VARS['avatargallery'] ) && !isset( $HTTP_POST_VARS['submitavatar'] ) && !isset( $HTTP_POST_VARS['cancelavatar'] ) )
        {
                ...
                $aim = htmlspecialchars(str_replace('+', ' ', $this_userdata['user_aim'] ), ENT_COMPAT);
                $msn = htmlspecialchars($this_userdata['user_msnm'], ENT_COMPAT);
                $yim = htmlspecialchars($this_userdata['user_yim'], ENT_COMPAT);

                $website = htmlspecialchars($this_userdata['user_website'], ENT_COMPAT);
                $location = htmlspecialchars($this_userdata['user_from'], ENT_COMPAT);
                $occupation = htmlspecialchars($this_userdata['user_occ'], ENT_COMPAT);
                $interests = htmlspecialchars($this_userdata['user_interests'], ENT_COMPAT);
                ...
                $user_dateformat = htmlspecialchars($this_userdata['user_dateformat'], ENT_COMPAT);
        ...
        if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
        {
                if( !$error )
                {
                        ...
                        {
                                $category = htmlspecialchars($HTTP_POST_VARS['avatarcategory'], ENT_COMPAT);
                        }
                        else
                        {
