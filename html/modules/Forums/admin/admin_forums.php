...
$subforum_rows=$forum_rows;
        ...
        for($i = 0; $i < $total_categories; $i++)
        {
                ...
                for($j = 0; $j < $total_forums; $j++)
                {
						...
						$forum_rows[$j]['forum_desc'] = ($forum_rows[$j]['forum_desc'] == '') ? '&nbsp;' : $forum_rows[$j]['forum_desc'];
				...
				if ($do_template)
				{
					...
					{
						$subforum_id = $subforum_rows[$k]['forum_id'];
						$subforum_rows[$k]['forum_desc'] = ($subforum_rows[$k]['forum_desc'] == '') ? '&nbsp;' : $subforum_rows[$k]['forum_desc'];
						if ($subforum_rows[$k]['attached_forum_id'] == $forum_id)
						{
							$template->assign_block_vars("catrow.forumrow", array(
								...
								'U_VIEWFORUM' => append_sid("../../../modules.php?name=Forums&amp;file=viewforum&amp;f=$subforum_id"),
