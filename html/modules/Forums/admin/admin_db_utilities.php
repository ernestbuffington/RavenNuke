...
include_once("../includes/sql_parse.php");
...
function get_table_def_postgresql($table, $crlf)
{
        ...
        $schema_create = str_replace(',' . $crlf . '$', '', $schema_create);
        $index_create = str_replace(',' . $crlf . '$', '', $index_create);
...
function get_table_content_postgresql($table, $handler)
{
		...
		$schema_insert = '';
                ...
                for($i = 0; $i < $i_num_fields; $i++)
                {
                        ...
                        elseif (preg_match("/date|timestamp/", $aryType[$i]))
                        {
				if (empty($strVal))
                                {
                                        $strQuote = "";
                                ...
                                if($sql_query != "")
                                {
                                        ...
                                        for($i = 0; $i < $sql_count; $i++)
                                        {
                                                ...
                                                if(!empty($sql) and $sql[0] != "#")
                                                {
                                                        ...
                                                        if(!$result && ( !(SQL_LAYER == 'postgresql' && stristr($sql, 'drop table') ) ) )
                                                        {
