...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $submit )
	{
		...
		$privmsg_subject = trim(htmlspecialchars($HTTP_POST_VARS['subject'], ENT_COMPAT));
	...
	else if ( $preview || $refresh || $error )
	{
		...
		//
		$to_username = (isset($HTTP_POST_VARS['username']) ) ? trim(htmlspecialchars(stripslashes($HTTP_POST_VARS['username'])), ENT_COMPAT) : '';

		$privmsg_subject = ( isset($HTTP_POST_VARS['subject']) ) ? trim(htmlspecialchars(stripslashes($HTTP_POST_VARS['subject']), ENT_COMPAT)) : '';
