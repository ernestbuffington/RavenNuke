...
/***************************************************************************
 * Added 10/9/2007 per email correspondence from Susann and Evaders
 ***************************************************************************/
if ($mode == "register") {
...
if (
        ...
        $mode == 'register' )
{
        include_once("modules/Forums/includes/functions_validate.php");
        ...
        if ( $mode == 'editprofile' )
        {
                ...
                $current_email = trim(htmlspecialchars($HTTP_POST_VARS['current_email'], ENT_COMPAT));
        }
	...
	$sid = $HTTP_POST_VARS['sid'] ?? 0;
        ...
        if ( !empty($HTTP_POST_VARS['language']) )
        {
                ...
                {
                        $user_lang = htmlspecialchars($HTTP_POST_VARS['language'], ENT_COMPAT);
                }
                else
                {
                        $error = true;
	...
	$board_config['default_dateformat'] = $row['config_value'];
        $user_dateformat = ( !empty($HTTP_POST_VARS['dateformat']) ) ? trim(htmlspecialchars($HTTP_POST_VARS['dateformat'], ENT_COMPAT)) : $board_config['default_dateformat'];

        $user_avatar_local = ( isset($HTTP_POST_VARS['avatarselect']) && !empty($HTTP_POST_VARS['submitavatar']) && $board_config['allow_avatar_local'] ) ? htmlspecialchars($HTTP_POST_VARS['avatarselect'], ENT_COMPAT) : ( ( isset($HTTP_POST_VARS['avatarlocal'])  ) ? htmlspecialchars($HTTP_POST_VARS['avatarlocal'], ENT_COMPAT) : '' );
        $user_avatar_category = ( isset($HTTP_POST_VARS['avatarcatname']) && $board_config['allow_avatar_local'] ) ? htmlspecialchars($HTTP_POST_VARS['avatarcatname'], ENT_COMPAT) : '' ;

        $user_avatar_remoteurl = ( !empty($HTTP_POST_VARS['avatarremoteurl']) ) ? trim(htmlspecialchars($HTTP_POST_VARS['avatarremoteurl'], ENT_COMPAT)) : '';
        ...
        if ( (isset($HTTP_POST_VARS['avatargallery']) || isset($HTTP_POST_VARS['submitavatar']) || isset($HTTP_POST_VARS['cancelavatar'])) && (!isset($HTTP_POST_VARS['submit'])) )
        {
                ...
                $cur_password = htmlspecialchars(stripslashes($cur_password), ENT_COMPAT);
                $new_password = htmlspecialchars(stripslashes($new_password), ENT_COMPAT);
                $password_confirm = htmlspecialchars(stripslashes($password_confirm), ENT_COMPAT);
                ...
                $signature = htmlspecialchars(stripslashes($signature), ENT_COMPAT);
...
{
        include_once("modules/Forums/includes/usercp_avatar.php");
	// session id check
	...
	if ($board_config['enable_confirm'] && $mode == 'register')
	{
		...
		{
			$confirm_id = htmlspecialchars($HTTP_POST_VARS['confirm_id'], ENT_COMPAT);
		...
		if ( !$error ) {
			...
			$sql = "INSERT INTO " . USERS_TABLE . "	(user_id, username, user_regdate, user_password, user_email, user_icq, user_website, user_occ, user_from, user_interests, user_sig, user_sig_bbcode_uid, user_avatar, user_avatar_type, user_viewemail, user_aim, user_yim, user_msnm, user_attachsig, user_allowsmile, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_popup_pm, user_timezone, user_dateformat, user_lang, user_style, user_level, user_allow_pm, user_active, user_actkey)
                            VALUES ($user_id, '" . str_replace("\'", "''", $username) . "', '" . addslashes($reg_date) . "', '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', '" . str_replace("\'", "''", $icq) . "', '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "', '" . str_replace("\'", "''", $interests) . "', '" . str_replace("\'", "''", $signature) . "', '$signature_bbcode_uid', $avatar_sql, $viewemail, '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', '" . str_replace("\'", "''", $yim) . "', '" . str_replace("\'", "''", $msn) . "', $attachsig, $allowsmilies, $allowhtml, $allowbbcode, $allowviewonline, $notifyreply, $notifypm, $popup_pm, $user_timezone, '" . str_replace("\'", "''", $user_dateformat) . "', '" . str_replace("\'", "''", $user_lang) . "', $user_style, 0, 1, ";
...
//added for modal popup - manits #0001562 
echo '<div style="display: none"><div id="ya_warning" align="center">' , OpenTable() , '<div align="center"><span style="font-weight: bold">' , $lang['Attention'] , '!</span>'
	, '<br /><br />' , sprintf($lang['YA_Warning'] , '<a href= "modules.php?name=Your_Account">Your Account</a>') , '.<br /><br /><a class="close" href="#">' , $lang['Continue'] , '</a>'
	, '&nbsp;|&nbsp;<a href= "modules.php?name=Your_Account&amp;op=edituser">' , $lang['Go_Your_Account'] , '</a></div>' , CloseTable() , '</div></div>';
...
if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
{
	...
	$avatar_category = ( !empty($HTTP_POST_VARS['avatarcategory']) ) ? htmlspecialchars($HTTP_POST_VARS['avatarcategory'], ENT_COMPAT) : '';
        ...
        display_avatar_gallery($mode, $avatar_category, $user_id, $email, $current_email, $coppa, $username, $new_password, $cur_password, $password_confirm, $icq, $aim, $msn, $yim, $website, $location, $occupation, $interests, $signature, $viewemail, $notifypm, $popup_pm, $notifyreply, $attachsig, $allowhtml, $allowbbcode, $allowsmilies, $allowviewonline, $user_style, $user_lang, $user_timezone, $user_dateformat, $userdata['session_id']);
}
else
{
        include_once("modules/Forums/includes/functions_selects.php");
	...
	//
		...
		'EMAIL' => $email ?? '',
                ...
                'BBCODE_STATUS' => sprintf($bbcode_status, '<a href="' . append_sid("faq.$phpEx?mode=bbcode") . '" target="phpbbcode">', '</a>'),
