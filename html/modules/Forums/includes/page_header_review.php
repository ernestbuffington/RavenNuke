...
while( $row = $db->sql_fetchrow($result) )
{
	...
	if ( $row['session_logged_in'] )
	{
		...
		if ( $row['user_id'] != $prev_user_id )
		{
			...
			{
				$row['username'] = '<span class="thick">' . $row['username'] . '</span>';
			...
			{
				$user_online_link = '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['user_id']) . '" ' . $style_color .'><span class="italic">' . $row['username'] . '</span></a>';
