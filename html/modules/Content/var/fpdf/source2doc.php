...
////
  ...
  $html = '<span style="font-weight:bold;">CLASSNAME:</span> ' . $classname . '<br />' . endl;
  if ($extends != '') $html .= '<span style="font-weight:bold;">EXTENDS:</span> ' . $extends . '<br />' . endl;
...
////
  ...
  for($i=0; $i < (is_countable($vname) ? count($vname) : 0) ; $i++)
  {
    ...
    if (empty($vtype[$i])) $html .= '<font size=2><span class="italic">(???)</span></font>' . endl;
    else $html .= '<font size=2><span class="italic">('. $vtype[$i] .')</span></font>' . endl;
    $html .= '</td>' . endl;

    $html .= '<td>' . endl;
    $html .= '<font size=2><span style="font-weight:bold;">var</span> ' . $vname[$i] . ';</font>' . endl;
    $html .= '</td>' . endl;
    $html .= '</tr>' . endl;
  }
...
/////
  ...
  for($i=0; $i < (is_countable($funcname) ? count($funcname) : 0) ; $i++)
  {
    ...
    $html .= '<font size=2><span style="font-weight:bold;">function</span> ' . $funcname[$i] . ')</font>' . endl;
    $html .= '</td>' . endl;
    $html .= '</tr>' . endl;
    ...
    if ($ret != '' or $desc != '')
    {
      ...
      if ($ret == '') $html .= '<font size=2><span style="font-weight:bold;">Return:</span> <span class="italic">?void?</span></font>' . endl;
      else $html .= '<font size=2><span style="font-weight:bold;">Return:</span> <span class="italic">' . trim($ret) . '</span></font>' . endl;
      $html .= '</td>' . endl;
      //@desc column
      $html .= '<td width="70%">' . endl;
      if ($desc == '') $html .= '<font size=2><span style="font-weight:bold;">OBS:</span> </font>' . endl;
      else $html .= '<font size=2><span style="font-weight:bold;">OBS:</span> ' . trim($desc) . '</font>' . endl;
      $html .= '</td>' . endl;
