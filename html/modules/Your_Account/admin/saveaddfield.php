...
/* Applied rules: Ernest Allen Buffington (TheGhost) 04/24/2023 9:17 PM
...
if (isset($field_name) && (is_countable($field_name) ? count($field_name) : 0) > 0) {
  foreach ($field_name as $key => $var) {
    $field_size[$key] = intval($field_size[$key]);
    $field_pos[$key] = intval($field_pos[$key]);
    $field_name[$key] = addslashes((string) $field_name[$key]);
    $field_value[$key] = addslashes((string) $field_value[$key]);
...
if ($mfield_name != "") {
  ...
  $mfield_name = addslashes((string) $mfield_name);
	$mfield_value = addslashes((string) $mfield_value);
