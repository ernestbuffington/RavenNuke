...
if($longip_hi < $longip_lo) {
  ...
  OpenTable();
  echo '<br />'."\n";
  echo '<div class="text-center"><strong>'._AB_HILOERROR.' </strong><br />'."\n";
  echo '<strong>'._GOBACK.'</strong></div><br />'."\n";
  CloseTable();
  include("footer.php");
  die();
...
$test1 = $db->sql_query("SELECT * FROM ".$prefix."_nsnst_ip2country WHERE (ip_lo<='$longip_lo' AND ip_hi>='$longip_lo') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
$test2 = $db->sql_query("SELECT * FROM ".$prefix."_nsnst_ip2country WHERE (ip_lo<='$longip_hi' AND ip_hi>='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
$test3 = $db->sql_query("SELECT * FROM ".$prefix."_nsnst_ip2country WHERE (ip_lo>='$longip_lo' AND ip_hi<='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
$test4 = $db->sql_query("SELECT * FROM ".$prefix."_nsnst_ip2country WHERE (ip_lo<='$longip_lo' AND ip_hi>='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
...
if($testnum1 > 0 OR $testnum2 >0 OR $testnum3 >0 OR $testnum4 >0) {
  $pagetitle = _AB_NUKESENTINEL.": "._AB_EDITIP2COUNTRYERROR;
  include("header.php");
  OpenTable();
  OpenMenu(_AB_EDITIP2COUNTRYERROR);
  mastermenu();
  CarryMenu();
  ip2cmenu();
  CloseMenu();
  ...
  echo $testmessage;
  echo '<div class="text-center"><strong>'._GOBACK.'</strong></div><br />'."\n";
  CloseTable();
  include("footer.php");
} else {
  $xtime = time();
  $db->sql_query("UPDATE `".$prefix."_nsnst_ip2country` SET `ip_lo`='$longip_lo', `ip_hi`='$longip_hi', `c2c`='$xc2c', `date`='$xtime' WHERE `ip_lo`='$old_ip_lo' AND `ip_hi`='$old_ip_hi'");
  $db->sql_query("UPDATE `".$prefix."_nsnst_tracked_ips` SET `c2c`='$xc2c' WHERE `ip_long` >= '$longip_lo' AND `ip_long` <= '$longip_hi'");
  $db->sql_query("UPDATE `".$prefix."_nsnst_blocked_ips` SET `c2c`='$xc2c' WHERE `ip_long` >= '$longip_lo' AND `ip_long` <= '$longip_hi'");
  $db->sql_query("UPDATE `".$prefix."_nsnst_blocked_ranges` SET `c2c`='$xc2c' WHERE `ip_lo` >= '$longip_lo' AND `ip_lo` <= '$longip_hi'");
