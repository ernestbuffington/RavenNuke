...
$test1 = $db->sql_query("SELECT * FROM ".$prefix."_nsnst_blocked_ranges WHERE (ip_lo<='$longip_lo' AND ip_hi>='$longip_lo') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
$test2 = $db->sql_query("SELECT * FROM ".$prefix."_nsnst_blocked_ranges WHERE (ip_lo<='$longip_hi' AND ip_hi>='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
$test3 = $db->sql_query("SELECT * FROM ".$prefix."_nsnst_blocked_ranges WHERE (ip_lo>='$longip_lo' AND ip_hi<='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
$test4 = $db->sql_query("SELECT * FROM ".$prefix."_nsnst_blocked_ranges WHERE (ip_lo<='$longip_lo' AND ip_hi>='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
...
if($testnum1 > 0 OR $testnum2 >0 OR $testnum3 >0 OR $testnum4 >0) {
  $pagetitle = _AB_NUKESENTINEL.": "._AB_EDITRANGEERROR;
  include("header.php");
  OpenTable();
  OpenMenu(_AB_EDITRANGEERROR);
  mastermenu();
  CarryMenu();
  blockedrangemenu();
  CloseMenu();
  ...
  if($ab_config['htaccess_path'] != "") {
    ...
    $old_masscidr = implode("", $old_masscidr);
    ...
    $ipfile = str_replace($old_masscidr, $new_masscidr, $ipfile);
    $ipfile = $ipfile;
    $doit = fopen($ab_config['htaccess_path'], "w");
    fwrite($doit, $ipfile);
    fclose($doit);
  }
  $db->sql_query("UPDATE `".$prefix."_nsnst_blocked_ranges` SET `ip_lo`='$longip_lo', `ip_hi`='$longip_hi', `c2c`='$xc2c', `date`='$xtime', `notes`='$xnotes', `expires`='$xexpires', `reason`='$xreason' WHERE `ip_lo`='$old_ip_lo' AND `ip_hi`='$old_ip_hi'");
