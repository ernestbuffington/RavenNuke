...
if(!isset($totalselected)) { $totalselected=0; }
if(!isset($min)) $min=0;
...
echo '<div class="text-center"><strong>'.$ip_addr.'</strong></div><br />'."\n";
// Page Sorting
$selcolumn1 = ($column == 'page') ? ' selected="selected"' : '';
$selcolumn2 = ($column == 'date') ? ' selected="selected"' : '';
$seldirection1 = ($direction == 'asc') ? ' selected="selected"' : '';
$seldirection2 = ($direction == 'desc') ? ' selected="selected"' : '';
...
while(list($ltid, $page, $date_time) = $db->sql_fetchrow($result)){
  ...
  echo '<td align="center" nowrap="nowrap"><a class="rn_csrf" href="'.$admin_file.'.php?op=ABTrackedDeleteSave&amp;tid='.$ltid.'&amp;user_id='.$user_id.'&amp;ip_addr='.$ip_addr.'&amp;column='.$column.'&amp;direction='.$direction.'&amp;min='.$min.'"><img src="images/nukesentinel/delete.png" height="16" width="16" border="0" alt="'._AB_DELETE.'" title="'._AB_DELETE.'" /></a></td>'."\n";
  echo '</tr>'."\n";
}
echo '</table>'."\n";
