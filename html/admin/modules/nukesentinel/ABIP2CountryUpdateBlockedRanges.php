...
if(!isset($min)) {
  ...
  $result = $db->sql_query("SELECT `ip_lo` FROM `".$prefix."_nsnst_blocked_ranges` LIMIT $min, $perpage");
  while(list($xip_lo) = $db->sql_fetchrow($result)) {
    ...
    if(!$xc2c) { $xc2c = "00"; }
    $db->sql_query("UPDATE `".$prefix."_nsnst_blocked_ranges` SET `c2c`='$xc2c' WHERE `ip_lo`='$xip_lo'");
  }
  $max=$min+$perpage;
  $pagesint = ($totalselected / $perpage);
  $pages = ceil($pagesint);
  ...
  echo '{'."\n";
  echo ' location.href = \''.$admin_file.'.php?op=ABIP2CountryUpdateBlockedRanges&min='.$max.csrf_rn_token().'\';'."\n";
  echo '}'."\n";
  echo '// --></script>'."\n";
  ...
  echo '<br />'."\n";
  OpenTable();
  echo '<div class="text-center"><strong>'._AB_IP2CUPDATEBLOCKEDRANGES.' '._AB_COMPLETED.'</strong></div>'."\n";
  CloseTable();
  include("footer.php");
}
