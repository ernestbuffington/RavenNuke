...
if(!isset($min)) {
  ...
  echo '{'."\n";
  echo ' location.href = \''.$admin_file.'.php?op=ABIP2CountryUpdateTracked&min='.$max.csrf_rn_token().'\';'."\n";
  echo '}'."\n";
  echo '// --></script>'."\n";
  ...
  echo '<br />'."\n";
  OpenTable();
  echo '<div class="text-center"><strong>'._AB_IP2CUPDATETRACKED.' '._AB_COMPLETED.'</strong></div>'."\n";
  CloseTable();
  include("footer.php");
}
