...
if(!isset($min)) {
  ...
  echo '<br />'."\n";
  echo '<form method="post" action="'.$admin_file.'.php?op=ABBlockedRangeOverlapCheck">'."\n";
  ...
  if($testmessage != "") {
    ...
    echo '<tr>'."\n";
    echo '<form action="'.$admin_file.'.php?op=ABBlockedRangeOverlapCheck" method="post">'."\n";
    echo '<td>'."\n";
    echo '<input type="hidden" name="min" value="'.$min.'" />'."\n";
    ...
    echo '{'."\n";
    echo ' location.href = \''.$admin_file.'.php?op=ABBlockedRangeOverlapCheck&min='.$max.csrf_rn_token().'\';'."\n";
    echo '}'."\n";
    echo '// --></script>'."\n";
  ...
  echo '<br />'."\n";
  OpenTable();
  echo '<div class="text-center"><strong>'._AB_BLOCKEDOVERLAPCHECK.' '._AB_COMPLETED.'</strong></div>'."\n";
  CloseTable();
  include("footer.php");
}
