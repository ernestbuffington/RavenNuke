...
include("header.php");
$sip = isset($sip) ? str_replace("X", "%", $sip) : '';
OpenTable();
...
if(isset($sip_lo[0]) && isset($sip_lo[1]) && isset($sip_lo[2]) && isset($sip_lo[3])) {
  if(($sip_lo[0] < 0 OR $sip_lo[0] > 255 OR !is_numeric($sip_lo[0])) OR ($sip_lo[1] < 0 OR $sip_lo[1] > 255 OR !is_numeric($sip_lo[1])) OR ($sip_lo[2] < 0 OR $sip_lo[2] > 255 OR !is_numeric($sip_lo[2])) OR ($sip_lo[3] < 0 OR $sip_lo[3] > 255 OR !is_numeric($sip_lo[3]))) {
    ...
    OpenTable();
    echo '<br />'."\n";
    echo '<div class="text-center"><strong>'._AB_LOERROR.' </strong><br />'."\n";
...
if(isset($sip_hi[0]) && isset($sip_hi[1]) && isset($sip_hi[2]) && isset($sip_hi[3])) {
  if(($sip_hi[0] < 0 OR $sip_hi[0] > 255 OR !is_numeric($sip_hi[0])) OR ($sip_hi[1] < 0 OR $sip_hi[1] > 255 OR !is_numeric($sip_hi[1])) OR ($sip_hi[2] < 0 OR $sip_hi[2] > 255 OR !is_numeric($sip_hi[2])) OR ($sip_hi[3] < 0 OR $sip_hi[3] > 255 OR !is_numeric($sip_hi[3]))) {
    ...
    OpenTable();
    echo '<br />'."\n";
    echo '<div class="text-center"><strong>'._AB_HIERROR.' </strong><br />'."\n";
    echo '<strong>'._GOBACK.'</strong></div><br />'."\n";
    CloseTable();
    include("footer.php");
    die();
...
if($totalselected > 0) {
  echo '<br />'."\n";
  OpenTable();
  echo '<div class="text-center title"><strong>'._AB_SEARCHBLOCKEDIPS.'</strong></div><br />'."\n";
  ...
  while($getIPs = $db->sql_fetchrow($result)) {
    ...
    echo '<a href="'.$admin_file.'.php?op=ABBlockedIPEdit&amp;xIPs='.$getIPs['ip_addr'].'&amp;xop='.$op.'&amp;sip='.$sip.'"><img src="images/nukesentinel/edit.png" border="0" alt="'._AB_EDIT.'" title="'._AB_EDIT.'" height="16" width="16" /></a>'."\n";
    echo '<a href="'.$admin_file.'.php?op=ABBlockedIPDelete&amp;xIPs='.$getIPs['ip_addr'].'&amp;xop='.$op.'&amp;sip='.$sip.'"><img src="images/nukesentinel/unblock.png" border="0" alt="'._AB_UNBLOCK.'" title="'._AB_UNBLOCK.'" height="16" width="16" /></a></td>'."\n";
    echo '</tr>'."\n";
  }
  echo '</table>'."\n";
  CloseTable();
...
//BLOCKED RANGES SEARCH RESULTS
  ...
  if($totalselected > 0) {
    echo '<br />'."\n";
    OpenTable();
    echo '<div class="text-center title"><strong>'._AB_SEARCHBLOCKEDRANGES.'</strong></div><br />'."\n";
    echo '<table summary="" align="center" border="0" cellpadding="2" cellspacing="2" bgcolor="'.$bgcolor2.'" width="100%">'."\n";
    echo '<tr bgcolor="'.$bgcolor2.'">'."\n";
    ...
    while($getIPs = $db->sql_fetchrow($result)) {
      ...
      echo '<td align="center"'.$valign.' nowrap="nowrap"><a href="'.$admin_file.'.php?op=ABBlockedRangeEdit&amp;ip_lo='.$getIPs['ip_lo'].'&amp;ip_hi='.$getIPs['ip_hi'].'&amp;sip='.$sip.'&amp;xop='.$op.'"><img src="images/nukesentinel/edit.png" border="0" alt="'._AB_EDIT.'" title="'._AB_EDIT.'" height="16" width="16" /></a>'."\n";
      echo '<a href="'.$admin_file.'.php?op=ABBlockedRangeDelete&amp;ip_lo='.$getIPs['ip_lo'].'&amp;ip_hi='.$getIPs['ip_hi'].'&amp;sip='.$sip.'&amp;xop='.$op.'"><img src="images/nukesentinel/unblock.png" border="0" alt="'._AB_UNBLOCK.'" title="'._AB_UNBLOCK.'" height="16" width="16" /></a></td>'."\n";
      echo '</tr>'."\n";
      $x++;
    }
    echo '</table>'."\n";
...
if($totalselected > 0) {
  echo '<br />'."\n";
  OpenTable();
  echo '<div class="text-center title"><strong>'._AB_SEARCHEXCLUDEDRANGES.'</strong></div><br />'."\n";
  echo '<table summary="" align="center" border="0" cellpadding="2" cellspacing="2" bgcolor="'.$bgcolor2.'" width="100%">'."\n";
  echo '<tr bgcolor="'.$bgcolor2.'">'."\n";
  echo '<td width="20%"><strong>'._AB_IPLO.'</strong></td>'."\n";
  echo '<td width="20%"><strong>'._AB_IPHI.'</strong></td>'."\n";
  ...
  while($getIPs = $db->sql_fetchrow($result)) {
    ...
    echo '<td align="center"'.$valign.'>'.date("Y-m-d", $getIPs['date']).'</td>'."\n";
    echo '<td align="center"'.$valign.'>'.$masscidr.'</td>'."\n";
    echo '<td align="center"'.$valign.' nowrap="nowrap"><a href="'.$admin_file.'.php?op=ABExcludedEdit&amp;ip_lo='.$getIPs['ip_lo'].'&amp;ip_hi='.$getIPs['ip_hi'].'&amp;sip='.$sip.'&amp;xop='.$op.'"><img src="images/nukesentinel/edit.png" border="0" alt="'._AB_EDIT.'" title="'._AB_EDIT.'" height="16" width="16" /></a>'."\n";
    echo '<a href="'.$admin_file.'.php?op=ABExcludedDelete&amp;ip_lo='.$getIPs['ip_lo'].'&amp;ip_hi='.$getIPs['ip_hi'].'&amp;sip='.$sip.'&amp;xop='.$op.'"><img src="images/nukesentinel/delete.png" border="0" alt="'._AB_DELETE.'" title="'._AB_DELETE.'" height="16" width="16" /></a></td>'."\n";
    echo '</tr>'."\n";
    $x++;
  }
  echo '</table>'."\n";
...
if($totalselected > 0) {
  echo '<br />'."\n";
  OpenTable();
  echo '<div class="text-center title"><strong>'._AB_SEARCHPROTECTEDRANGES.'</strong></div><br />'."\n";
  echo '<table summary="" align="center" border="0" cellpadding="2" cellspacing="2" bgcolor="'.$bgcolor2.'" width="100%">'."\n";
  echo '<tr bgcolor="'.$bgcolor2.'">'."\n";
  echo '<td width="20%"><strong>'._AB_IPLO.'</strong></td>'."\n";
  echo '<td width="20%"><strong>'._AB_IPHI.'</strong></td>'."\n";
  ...
  while($getIPs = $db->sql_fetchrow($result)) {
    ...
    echo '<td align="center"'.$valign.'>'.date("Y-m-d", $getIPs['date']).'</td>'."\n";
    echo '<td align="center"'.$valign.'>'.$masscidr.'</td>'."\n";
    echo '<td align="center"'.$valign.' nowrap="nowrap"><a href="'.$admin_file.'.php?op=ABProtectedEdit&amp;ip_lo='.$getIPs['ip_lo'].'&amp;ip_hi='.$getIPs['ip_hi'].'&amp;sip='.$sip.'&amp;xop='.$op.'"><img src="images/nukesentinel/edit.png" border="0" alt="'._AB_EDIT.'" title="'._AB_EDIT.'" height="16" width="16" /></a>'."\n";
    echo '<a href="'.$admin_file.'.php?op=ABProtectedDelete&amp;ip_lo='.$getIPs['ip_lo'].'&amp;ip_hi='.$getIPs['ip_hi'].'&amp;sip='.$sip.'&amp;xop='.$op.'"><img src="images/nukesentinel/delete.png" border="0" alt="'._AB_DELETE.'" title="'._AB_DELETE.'" height="16" width="16" /></a></td>'."\n";
    echo '</tr>'."\n";
    $x++;
  }
  echo '</table>'."\n";
...
if($totalselected > 0) {
  echo '<br />'."\n";
  OpenTable();
  echo '<div class="text-center title"><strong>'._AB_IP2CSEARCH.'</strong></div><br />'."\n";
  echo '<table summary="" align="center" border="0" cellpadding="2" cellspacing="2" bgcolor="'.$bgcolor2.'" width="100%">'."\n";
  echo '<tr bgcolor="'.$bgcolor2.'">'."\n";
  echo '<td width="20%"><strong>'._AB_IPLO.'</strong></td>'."\n";
  echo '<td width="20%"><strong>'._AB_IPHI.'</strong></td>'."\n";
  ...
  while($getIPs = $db->sql_fetchrow($result)) {
    ...
    echo '<a href="'.$admin_file.'.php?op=ABIP2CountryEdit&amp;ip_lo='.$getIPs['ip_lo'].'&amp;ip_hi='.$getIPs['ip_hi'].'&amp;sip='.$sip.'&amp;xop='.$op.'"><img src="images/nukesentinel/edit.png" border="0" alt="'._AB_EDIT.'" title="'._AB_EDIT.'" height="16" width="16" /></a>'."\n";
    echo '<a href="'.$admin_file.'.php?op=ABIP2CountryDelete&amp;ip_lo='.$getIPs['ip_lo'].'&amp;ip_hi='.$getIPs['ip_hi'].'&amp;sip='.$sip.'&amp;xop='.$op.'"><img src="images/nukesentinel/delete.png" border="0" alt="'._AB_DELETE.'" title="'._AB_DELETE.'" height="16" width="16" /></a></td>'."\n";
    echo '</tr>'."\n";
    $x++;
  }
  echo '</table>'."\n";
