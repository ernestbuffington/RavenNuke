...
$test1 = $db->sql_query("SELECT * FROM `".$prefix."_nsnst_blocked_ranges` WHERE `ip_lo`<='$longip_lo' AND `ip_hi`>='$longip_lo' ORDER BY ip_lo");
$test2 = $db->sql_query("SELECT * FROM `".$prefix."_nsnst_blocked_ranges` WHERE `ip_lo`<='$longip_hi' AND `ip_hi`>='$longip_hi' ORDER BY ip_lo");
$test3 = $db->sql_query("SELECT * FROM `".$prefix."_nsnst_blocked_ranges` WHERE `ip_lo`>='$longip_lo' AND `ip_hi`<='$longip_hi' ORDER BY ip_lo");
$test4 = $db->sql_query("SELECT * FROM `".$prefix."_nsnst_blocked_ranges` WHERE `ip_lo`<='$longip_lo' AND `ip_hi`>='$longip_hi' ORDER BY ip_lo");
...
if($testnum1 > 0 OR $testnum2 >0 OR $testnum3 >0 OR $testnum4 >0) {
  ...
  $db->sql_query("INSERT INTO `".$prefix."_nsnst_blocked_ranges` VALUES ('$longip_lo', '$longip_hi', '$xtime', '$xnotes', '$xreason', '$xexpires', '$xc2c')");
  if($another == 1) {
    header("Location: ".$admin_file.".php?op=ABBlockedRangeAdd");
  }else {
