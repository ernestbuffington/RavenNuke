...
if($totalselected > 0) {
  ...
  while(list($refered_from, $tid, $lastview, $hits) = $db->sql_fetchrow($result)){
    ...
    if($rfrom != "on site" AND $rfrom != "none" AND $rfrom !="local") {
      $rfrom = '<a href="includes/nsbypass.php?tid='.$tid.'" target="_blank" title="'.$refered_from.'">'.htmlspecialchars($rfrom, ENT_QUOTES, _CHARSET).'</a>';
    } else {
      $rfrom = $rfrom;
    }
    ...
    echo '<a class="rn_csrf" href="'.$admin_file.'.php?op=ABTrackedRefersDelete&amp;tid='.$tid.'&amp;min='.$min.'&amp;column='.$column.'&amp;direction='.$direction.'"><img src="images/nukesentinel/delete.png" height="16" width="16" alt="'._AB_DELETE.'" title="'._AB_DELETE.'" border="0" /></a></td>'."\n";
    echo '</tr>'."\n";
  }
  echo '</table>'."\n";
