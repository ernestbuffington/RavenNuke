 ...
 *
 * Applied rules: Ernest Allen Buffington (TheGhost) 04/22/2023 8:41 PM
...
abstract class elFinderVolumeDriver {
  ...
  **/
 private static bool $mimetypesLoaded = false;
	...
	public function mount(array $opts) {
		...
		$this->separator = $this->options['separator'] ?? DIRECTORY_SEPARATOR;
		if (! empty($this->options['winHashFix'])) {
			$this->separatorForHash = ($this->separator !== '/') ? '/' : '';
		}
		$this->systemRoot = $this->options['systemRoot'] ?? $this->separator;
		...
		if (!empty($this->options['attributes']) && is_array($this->options['attributes'])) {
			...
			foreach ($this->options['attributes'] as $a) {
				...
				if (!empty($a['pattern']) || (is_countable($a) ? count($a) : 0) > 1) {
					$this->attributes[] = $a;
				}
			}
		}
		...
		if (is_string($this->options['uploadOrder'])) { // telephat_mode on, compatibility with 1.x
			$parts = explode(',', $this->options['uploadOrder'] ?? 'deny,allow');
		...
		if ($this->tmpLinkPath && !$this->tmpLinkUrl) {
			$cur = realpath('./');
			$i = 0;
			while($cur !== $this->systemRoot && !str_starts_with($this->tmpLinkPath, $cur)) {
				$i++;
				$cur = dirname($cur);
			}
	...
	public function defaultPath() {
		return $this->encode($this->startPath ?: $this->root);
	}
	...
	public function options($hash) {
		...
		$opts = array(
			...
			'uploadMime'      => array(
				'firstOrder' => $this->uploadOrder[0] ?? 'deny',
	...
	public function getOptionsPlugin($name = '') {
		if ($name) {
			return $this->options['plugin'][$name] ?? array();
		} else {
	...
	public function file($hash) {
		...
		return $file ?: $this->setError(elFinder::ERROR_FILE_NOT_FOUND);
	}
	...
	public function zipdl($hashes) {
		...
		if ($dir = $this->getItemsInHand($hashes)) {
			$tmppre = (str_starts_with(PHP_OS, 'WIN'))? 'zdl' : 'elfzdl';
			$pdir = dirname($dir);
	...
	public function extract($hash, $makedir = null) {
		...
		$archiver = $this->archivers['extract'][$file['mime']] ?? false;
	...
	public function archive($hashes, $mime, $name = '') {
		...
		$archiver = $this->archivers['create'][$mime] ?? false;
	...
	public function resize($hash, $width, $height, $x, $y, $mode = 'resize', $bg = '', $degree = 0, $jpgQuality = null) {
		...
		switch($mode) {
			...
			case 'fitsquare':
				$result = $this->imgSquareFit($work_path, $width, $height, 'center', 'middle', ($bg ?: $this->options['tmbBgColor']), null, $jpgQuality);
				break;

			case 'rotate':
				$result = $this->imgRotate($work_path, $degree, ($bg ?: $this->options['bgColorFb']), null, $jpgQuality);
				break;
	...
	public function getTempLinkInfo($name = null) {
		if ($this->tmpLinkPath) {
			...
			} else if (!str_starts_with($name, 'temp_')) {
				$name = 'temp_' . $name;
			}
	...
	protected function decode($hash) {
		if (str_starts_with($hash, $this->id)) {
	...
	protected function nameAccepted($name, $isDir = false) {
		if (json_encode($name, JSON_THROW_ON_ERROR)===false) {
			return false;
		}
	...
	protected function getRootstatCachekey() {
		return md5($this->root.($this->options['alias'] ?? ''));
	}
	...
	protected function stat($path) {
		...
		$ret = $this->cache[$path] ?? $this->updateCache($path, $this->convEncOut($this->_stat($this->convEncIn($path))));
	...
	protected function updateCache($path, $stat) {
		...
		// name check
		if (!$jeName = json_encode($stat['name'], JSON_THROW_ON_ERROR)) {
			return $this->cache[$path] = array();
		}
		// fix name if required
		if ($this->options['utf8fix'] && $this->options['utf8patterns'] && $this->options['utf8replace']) {
			$stat['name'] = json_decode(str_replace($this->options['utf8patterns'], $this->options['utf8replace'], $jeName), null, 512, JSON_THROW_ON_ERROR);
		}
		...
		if ($stat['read'] && empty($stat['hidden'])) {
			...
			if (!isset($stat['url']) && $this->URL && $this->encoding) {
				...
				$stat['url'] = rtrim($this->URL, '/') . '/' . str_replace('%2F', '/', rawurlencode((str_starts_with(PHP_OS, 'WIN'))? $_path : $this->convEncIn($_path, true)));
			}
		} else {
			if ($isDir) {
				unset($stat['dirs']);
			...
			if ($file && file_exists($file)) {
				...
				foreach ($mimecf as $line_num => $line) {
					if (!preg_match('/^\s*#/', $line)) {
						...
						for ($i = 1, $size = is_countable($mime) ? count($mime) : 0; $i < $size ; $i++) {
	...
	protected function isMyReload($target = '', $ARGtarget = '') {
		...
		if (! empty($this->ARGS['reload'])) {
			if ($ARGtarget === '') {
				$ARGtarget = $this->ARGS['target'] ?? ((isset($this->ARGS['targets']) && is_array($this->ARGS['targets']) && count($this->ARGS['targets']) === 1)?
						$this->ARGS['targets'][0] : '');
			}
			if ($ARGtarget !== '') {
				$ARGtarget = strval($ARGtarget);
				if ($target === '') {
					return (str_starts_with($ARGtarget, $this->id));
				} else {
	...
	protected function copyFrom($volume, $src, $destination, $name) {
		...
		if ($srcIsDir) {
			...
			if (str_starts_with($source['mime'], 'image') && ($dim = $volume->dimensions($src))) {
				if (is_array($dim)) {
					$dim = $dim['dim'] ?? null;
				}
				if ($dim) {
	...
	protected function gettmb($path, $stat) {
		if ($this->tmbURL && $this->tmbPath) {
			...
			if (str_starts_with($path, $this->tmbPath)) {
  ...
  if (($s = getimagesize($path)) == false || $degree % 360 === 0) {
	...
	protected function imageMagickConvertPrepare($path, $destformat, $jpgQuality, $imageSize = null) {
		...
		if ($this->procExec($cmd, $o) === 0) {
			$ani = preg_split('/(?:\r\n|\n|\r)/', trim($o));
			if ((is_countable($ani) ? count($ani) : 0) < 2) {
				$ani = false;
			}
		}
	...
	**/
	...
	protected function getArchivers($use_cache = true) {
		...
		if ($this->procExec('') === 0) {
			...
			if ($c == 0) {
				...
				if (str_starts_with(PHP_OS, 'WIN') && empty($arcs['extract']['application/x-rar'])) {
	...
	protected function getFullPath($path, $base) {
		...
		if ($base[0] === $separator && strpos($base, (string) 0, strlen($systemroot)) !== $systemroot) {
