...
FeedCreator class v1.8.0-nukeSEO (development)
http://nukeSEO.com
...
/* Applied rules: Ernest Allen Buffington (TheGhost) 04/22/2023 9:00 Pm
...
define("FEEDCREATOR_VERSION", "FeedCreator 1.8.0-nukeFEED");
...
class FeedItem extends HtmlDescribable {
	 ...
	 */
	public $author, $authorEmail, $authorURL,$image, $category, $comments, $guid, $source, $creator, $contributor;
	...
	function output() {
		...
		if (!$this->rawFieldContent) {
			...
			if ($this->truncSize and is_int($this->truncSize)) {
				$result = (new FeedCreator())->iTrunc(htmlspecialchars($this->rawFieldContent), $this->truncSize);
			} else {
...
class UniversalFeedCreator extends FeedCreator {
	...
	function _setMIME() {
		header('Content-type: ' . $this->_feed->contentType .'; charset=' . $this->encoding, true);
	}
...
class FeedCreator extends HtmlDescribable {
	...
	* Ignored in the output when empty.
	 ...
	 */
	 ...
	 public $generator = "http://nukeSEO.com";
	...
	function saveFeed($filename="", $displayContents=true) {
		...
		if ($feedFile) {
			...
			echo '<br /><span class="thick">Error creating feed file, please check write permissions.</span><br />';
		}
	}
 ...
 */
 class AtomCreator10 extends FeedCreator {
	...
	function createFeed() {
		...
		$feed.= ">\n";
		$feed.= "    <title>".htmlspecialchars($this->title)."</title>\n";
		if (!empty($this->description)) $feed.= "    <subtitle>".htmlspecialchars($this->description)."</subtitle>\n";
		$feed.= "    <link rel=\"alternate\" type=\"text/html\" href=\"".htmlspecialchars($this->link)."\"/>\n";
		$feed.= "    <id>".htmlspecialchars($this->link)."</id>\n";
		$now = new FeedDate();
		$feed.= "    <updated>".htmlspecialchars($now->iso8601())."</updated>\n";
		if (empty($this->editor)) $this->editor = 'noone@example.com';
		if ($this->editor!="") {
			$feed.= "    <author>\n";
...
class MBOXCreator extends FeedCreator {
  ...
  for ($i=0;$i<(is_countable($this->items) ? count($this->items) : 0);$i++) {
			...
			$feed.= "\n";
			$body = chunk_split((new MBOXCreator())->qp_enc($this->items[$i]->description));
			$feed.= preg_replace("~\nFrom ([^\n]*)(\n?)~","\n>From $1$2\n",$body);
			$feed.= "\n";
			$feed.= "\n";
		}
		...
		for ($i=0;$i<(is_countable($this->items) ? count($this->items) : 0);$i++) {
			...
			if ($this->items[$i]->title) {
				if ($this->items[$i]->link) {
					$feedArray[] =
						"<div class='".$this->stylePrefix."item_title'><a href='".htmlentities($this->items[$i]->link)."' class='".$this->stylePrefix.
